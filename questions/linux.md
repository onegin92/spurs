...
Как открыть и пробросить порт tcp 80 на Linux
<details>
  <summary>Ответ</summary>
iptables -A INPUT -p tcp -d 10.10.1.1/32 --dport 80 -j ACCEPT
iptables -A PREROUTING -t nat -p tcp -d 10.10.1.1 --dport 80 -j REDIRECT --to-ports 80
iptables -L -n
</details>
...
<details>
  <summary>Ответ</summary>
ps -eo user,pid,pcpu,nice,comm # смотрим текущий приоритет
renice 15 -p id # меняем, например на 15
</details>

Назовите основные цепочки в iptables, за что они отвечают?
<details>
  <summary>Ответ</summary>
INPUT - применяется к пакетам, которые предназначены для самой системы.
FORWARD - применяется к пакетам, которые проходят через систему.
OUTPUT - применяется к пакетам, которые исходят из системы.
</details>
